...
int l2ping(char *svr, int debug, int cont) {
  ...
  if (debug)
    printf("[*] l2ping ping: %s from %s (data size %d) ...\n", svr, str, size);
  ...
  while (count == -1 || count-- > 0) {
    ...
    while (1) {
      ...
      if ((err = recv(s, buf, sizeof(buf), 0)) < 0) {
        ...
        if (cont) {
          if (debug)
            fprintf(stderr, "[d] l2ping: continue anyway\n");
          close(s);
          return 1;
        }
        // exit(1);
    ...
    if (!lost) {
      ...
      if (debug)
        fprintf(stdout, "[*] l2ping: %d bytes from %s id %d time %.2fms\n",
      ...
      if (debug)
        fprintf(stdout, "[!] l2ping: no response from %s: id %d\n", svr, id);
      return 0;
    }
