...
#define LENGTH 20
#define BUFCODE 100
#define SIZE_L2HEADER                                                          \
  10 // Change it there if you want :
     // Size of len in L2CAP header (mode full L2CAP header fuzzing)
#define MAX_L2CAP_FIELDS                                                       \
  0x1F // Max value here for "full header fuzzing" is 0xFF but fuzz are tooooooo
       // long with it....
#define DEFAULT_DELAY 500 // Default delay in miliseconds

// variables
int pdelay = DEFAULT_DELAY;
int dontstop = 0, vdebug = 0, shutnop = 0, silent = 0, filegen = 0, filecnt = 0,
    noping = 0;
...
int genreplay(char *payload, int savedsize) {
  ...
  strftime(strOutFile, 240, "replay_packet/replay_l2cap_packet_%d%m%Y%H%M%S.",
           ptr);
  ...
  if (hSrc == NULL) {
    fprintf(stdout, "[!] bss: failed to open source file "
                    "'replay_packet/replay_packet.c'\n");
    return 1;
  }
  ...
  if (charFileBuffer == NULL) {
    fprintf(stdout, "[!] bss: couldn't allocate memory for replay_packet file "
                    "generation\n");
    fclose(hSrc);
    fclose(hDst);
    return 1;
  ...
  if (vdebug)
    fprintf(stdout, "[d] bss: wrote %s ok\n", strOutFile);

  // tidy
  if (vdebug)
    fprintf(stdout, "[d] bss: free'ing crashpacket\n");
  free(crashpacket);
  if (vdebug)
    fprintf(stdout, "[d] bss: free'ing charFileBuffer\n");
  free(charFileBuffer);
  return 0;
}
...
void l2dos(char *src_bdaddr, char *bdstr_addr, int cmdnum, int siz, char pad) {
  ...
  if (vdebug)
    fprintf(stdout, "[d] l2dos called with cmdnum %d\n", cmdnum);
  ...
  if (vdebug)
    fprintf(stdout, "[d] malloc'ing buf\n");
...
recoverl2dos:
  if (vdebug)
    fprintf(stdout, "[d] bss: creating socket\n");
  ...
  if (vdebug)
    fprintf(stdout, "[d] bss: connecting\n");
  memset(&addr, 0, sizeof(addr));
  addr.l2_family = AF_BLUETOOTH;
  ...
  if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    if (dontstop) {
      ...
      if (vdebug)
        fprintf(stdout, "[!] bss: couldn't connect\n");
      if (vdebug)
        fprintf(stdout, "[d] bss: jumping to recoverl2dos\n");
      goto recoverl2dos;
    } else {
      if (silent)
        fprintf(stdout, "\n");
  ...
  for (i = 0; i < IT; i++) { // Send IT times the packet thru the air
    if (vdebug)
      fprintf(stdout, "[d] bss: next l2dos i - %d\n", i);
    ...
    if (send(sock, buf, siz ? siz : MAXSIZE, 0) <= 0) {
      /* Now we check after each sending failure if the host is still responding
       * the pings */
      ...
      if (noping == 0) {
        if (!l2ping(bdstr_addr, vdebug, dontstop)) {
          ...
          if (!silent)
            fprintf(stdout, "[*] bss: l2ping returned the hose is up!\n");
        }

        if (!silent) {
          fprintf(stdout,
                  "[i] potential crash detected for %s, check l2ping response "
                  "above\n",
                  bdstr_addr);
        }
        ...
        if (!silent) {
          fprintf(stdout,
                  "[i] potential crash detected for %s, check with an inquiry "
                  "scan\n",
                  bdstr_addr);
        }
      }
      ...
      if (deadstop) {
        fprintf(stdout, "[!] bss: shutting down due to use of -x option - "
                        "hosts l2cap stack in stack \n");
        exit(EXIT_SUCCESS);
      }
    }
    if (++id > 254)
  ...
  if (vdebug)
    fprintf(stdout, "[d] free'ing strcode\n");
  free(strcode);
  if (vdebug)
    fprintf(stdout, "[d] free'ing buf\n");
  free(buf);
}
...
void l2fuzz(char *bdstr_addr, int maxsize, int maxcrash) {
  ...
  fprintf(stdout, "[i] performing full random L2CAP fuzzing, take a seat and "
                  "crack open a beer...\n");

  if (noping == 0) {
    // if Debugging is turned on just check the host is actually responding to
    // l2cap pings
    if (vdebug) {
      if (!l2ping(bdstr_addr, vdebug, dontstop)) {
        fprintf(stdout, "[d] [!] bss: (entry to l2fuzz) l2ping returned that "
                        "the host is down!\n");
      } else {
        fprintf(stdout, "[d] [*] bss: (entry to l2fuzz) l2ping returned that "
                        "the host is up!\n");
...
recoverl2fuzz:
  ...
  if (vdebug)
    fprintf(stdout, "[d] bss: malloc'ing savedbuf\n");
  ...
  while (1) // Initite loop (ctrl-c to stop...)
  {
    ...
    if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
      if (dontstop) {
        ...
        if (vdebug)
          fprintf(stdout, "[!] bss: couldn't connect.\n");
        if (vdebug)
          fprintf(stdout, "[d] bss: jumping to recover2fuzz\n");
        goto recoverl2fuzz;
      } else {
        if (silent)
          fprintf(stdout, "\n");
      ...
      if (vdebug)
        fprintf(stdout, "[d] bss: connected\n");
    }
    ...
    if (sndsize <= 0) {
      if (vdebug)
        fprintf(stdout, "[d] bss: send size was <= 0\n");
      // Now we check after each sending failure if the host is still responding
      // the pings
      if ((!silent) && (!vdebug))
        fprintf(stdout, "\n");
      ...
      // if it's not runnning in client mode or a bug was detected while -x was
      // supplied
      if ((!silent) || (deadstop)) {
        if (noping == 0) {
          fprintf(stdout,
                  "[i] bss: potential crash detected for %s, check l2ping "
                  "response above\n",
                  bdstr_addr);
        } else {
          fprintf(stdout,
                  "[i] bss: potential crash detected for %s, check with an "
                  "inquiry scan\n",
                  bdstr_addr);
        }
      ...
      if (filegen) {
        // if told to close on crash and crash is detect OR if not told to close
        // on crash
        if (((shutnop) && (deadstop)) || (!shutnop)) {
          if (vdebug)
            fprintf(stdout, "[d] generating replay_packet\n");
          if (genreplay(savedbuf, savedsize) == 0) {
            if (!silent)
              fprintf(stdout, "[i] generated replay_packet ok!\n");
            if (silent)
              fprintf(stdout, "G");
          } else {
            if (!silent)
              fprintf(stdout, "[!] error generating replay file\n");
          }
        }
      }
      ...
      if ((crash_count == maxcrash) && (maxcrash > 0)) {
        ...
        free(buf);
        if (vdebug)
          fprintf(stdout, "[d] free'ing savedbuf\n");
        free(savedbuf);
        exit(EXIT_SUCCESS);
      }
      ...
      if (deadstop) {
        fprintf(stdout, "[!] shutting down due to use of -x option - hosts "
                        "l2cap stack in stack\n");
        exit(EXIT_SUCCESS);
      }

    } else { // we sent some data
      if (vdebug)
        fprintf(stdout, "[d] send size was > 0\n");
      if ((vdebug) && (!silent)) {
        if (vdebug)
          fprintf(stdout, "\n");
    ...
    savedsize = size;
    if (vdebug)
      fprintf(stdout, "[d] bss: free'ing buf\n");
    free(buf);
    if (savedbuf != NULL) {
      if (vdebug)
        fprintf(stdout, "[d] bss: free'ing savedbuf\n");
      free(savedbuf);
    }

    // Be tidy
    if (vdebug)
      fprintf(stdout, "[d] bss: closing socket\n");
    close(sock);
    if (vdebug)
      fprintf(stdout, "[d] bss: jumping to recoverl2fuzz\n");
    goto recoverl2fuzz;
  }
}
...
void rfcfuzz(char *bdstr_addr, int maxsize, int maxcrash, int ch) {
  ...
  fprintf(stdout, "[i] performing full random RFCOMM fuzzing, take a seat and "
                  "crack open a beer...\n");
  ...
  if (noping == 0) {
    ...
    if (vdebug) {
      if (!l2ping(bdstr_addr, vdebug, dontstop)) {
        fprintf(stdout, "[d] [!] bss: (entry to rfcfuzz) l2ping returned that "
                        "the host is down!\n");
      } else {
        fprintf(stdout, "[d] [*] bss: (entry to rfcfuzz) l2ping returned that "
                        "the host is up!\n");
    ...
    if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
      if (dontstop) {
        ...
        if (vdebug)
          fprintf(stdout, "[!] bss: couldn't connect.\n");
        if (vdebug)
          fprintf(stdout, "[d] bss: jumping to recoverrfcfuzz\n");
        goto recoverrfcfuzz;
      } else {
        if (silent)
          fprintf(stdout, "\n");
...
void l2fuzz_header(char *bdstr_addr, int payload_size, char pad) {
  ...
  if (vdebug)
    fprintf(stdout, "[d] bss: malloc'ing buf\n");
  ...
  if (vdebug)
    fprintf(stdout, "[d] bss: malloc'ing savedbuf\n");
  if (!(savedbuf =
            (char *)malloc((int)payload_size + L2CAP_CMD_HDR_SIZE + 1))) {
    perror("[!] bss: can't malloc");
    exit(EXIT_FAILURE);
  }
...
recoverl2fuzzhdr:
  ...
  if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    if (dontstop) {
      ...
      if (vdebug)
        fprintf(stdout,
                "[!] bss: couldn't connect . jumping to recoverl2fuzzhdr\n");
  ...
  for (len = 0; len < SIZE_L2HEADER; len++) {
    if (vdebug)
      fprintf(stdout, "[d] bss: next l2fuzzhdr len %d\n", len);
    for (code = 0; code < MAX_L2CAP_FIELDS; code++) {
      if (vdebug)
        fprintf(stdout, "[d] bss: next l2fuzzhdr code %d\n", code);
      for (id = 0; id < MAX_L2CAP_FIELDS; id++) {
        if (vdebug)
          fprintf(stdout, "[d] bss: next l2fuzzhdr id %d\n", id);
        usleep(pdelay * 1000);
        cmd = (l2cap_cmd_hdr *)buf;
        cmd->code = code;
        ...
        if ((sndsize = send(sock, buf, L2CAP_CMD_HDR_SIZE + payload_size, 0)) <=
            0) {
          /* Now we check after each sending failure if the host is still
           * responding the pings */
          ...
          if (noping == 0) {
            if (!l2ping(bdstr_addr, vdebug, dontstop)) {
              fprintf(stdout,
                      "[!] bss: l2ping returned that the host is down!\n");
              if (shutnop)
                deadstop = 1;
              goto recoverl2fuzzhdr;
            } else {
          ...
          // if file generation is on
          if (filegen) {
            // if told to close on crash and crash is detect OR if not told to
            // close on crash
            if (((shutnop) && (deadstop)) || (!shutnop)) {
              if (vdebug)
                fprintf(stdout, "[d] generating replay_packet.c\n");
              if (genreplay(savedbuf, L2CAP_CMD_HDR_SIZE + payload_size) == 0) {
                if (!silent)
                  fprintf(stdout, "[d] generated ok!\n");
                if (silent)
                  fprintf(stdout, "G");
              } else {
                if (!silent)
...
int usage(char *name) {
  fprintf(stderr,
          "Usage: %s\t[-i iface] [-d delay] [-c] [-v] [-x] [-P0] [-q] [-o]\n",
          name);
  fprintf(
      stderr,
      "\t\t[-s size] [-m mode] [-p pad_byte] [-M maxcrash_count] <bdaddr>\n\n");
  fprintf(stderr, "[-i iface] - Optional output interface (format hci[X] - "
                  "check 'hciconfig -a')\n");
  fprintf(stderr,
          "[-d delay] - Optional delay (miliseconds). Default is %dms\n",
          DEFAULT_DELAY);
  fprintf(stderr, "[-c]       - Continue even on errors we would normally exit "
                  "on (except malloc)\n");
  fprintf(stderr, "             This overrides -x in most places\n");
  fprintf(stderr, "[-v]       - Verbose debugging\n");
  fprintf(stderr, "[-x]       - Exit on potential crashes that also don't "
                  "respond to secondary l2ping's\n");
  fprintf(stderr, "[-P0]      - Do not perform L2CAP ping (some hosts don't "
                  "respond to such packets\n");
  fprintf(stderr, "             This overrides -x in most places\n");
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < argc; i++) {
    if (strchr(argv[i], ':'))
      ...
      if (!memcmp(argv[i], "-i", 2) &&
          (pouet = hci_devba(atoi(argv[++i] + 3), &local_bdaddr)) < 0)
        usage(argv[0]);
      ...
      if (!memcmp(argv[i], "-P0", 3) && (noping == 0)) {
        fprintf(stdout, "[*] Do not use L2CAP ping (some hosts don't respond "
                        "to such packets): on\n");
        noping = 1;
      }
  ...
  fprintf(stdout, "\n[*do a little dance*] Your bluetooth device didn't crash "
